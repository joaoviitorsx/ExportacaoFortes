...
def notificacaoProgresso(page: ft.Page) -> ft.Container:
    estilo = {
        "bg": "#3474dc",
        "text": "white"
    }
    ...
    page.overlay.append(progresso_card)
    page.update()
    ...
    def animarEntrada():
        import time
        time.sleep(0.1)
        progresso_card.opacity = 1
        progresso_card.offset = ft.Offset(0, 0)
        page.update()

    threading.Thread(target=animarEntrada, daemon=True).start()

    return progresso_card